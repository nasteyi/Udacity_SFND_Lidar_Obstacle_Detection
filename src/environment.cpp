...
void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer, ProcessPointClouds<pcl::PointXYZI>& pointProcessor,
    ...
    inputCloud =
        pointProcessor.FilterCloud(inputCloud, 0.3, Eigen::Vector4f(-10, -5, -2, 1), Eigen::Vector4f(30, 8, 1, 1));

    std::pair<pcl::PointCloud<pcl::PointXYZI>::Ptr, pcl::PointCloud<pcl::PointXYZI>::Ptr> segmentCloud =
        pointProcessor.SegmentPlane(inputCloud, 25, 0.3f);
    ...
    for (const auto& cluster : cloudClusters) {
        ...
        renderPointCloud(viewer, cluster, "obstaclesCloud" + strClusterId, colors.at(clusterId % colors.size()));
...
void simpleHighway(pcl::visualization::PCLVisualizer::Ptr& viewer) {
    ...
    std::unique_ptr<ProcessPointClouds<pcl::PointXYZ>> pointProcessor(new ProcessPointClouds<pcl::PointXYZ>());

    std::pair<typename pcl::PointCloud<pcl::PointXYZ>::Ptr, typename pcl::PointCloud<pcl::PointXYZ>::Ptr> segmentCloud =
