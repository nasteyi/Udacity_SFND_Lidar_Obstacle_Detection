...
void renderPointCloud(pcl::visualization::PCLVisualizer::Ptr& viewer, const pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud,
    ...
    if (color.r == -1) {
        ...
        viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, color.r, color.g, color.b,
                                                 name);
    }
...
void renderBox(pcl::visualization::PCLVisualizer::Ptr& viewer, const Box& box, int id, Color color, float opacity) {
