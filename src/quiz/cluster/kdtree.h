...
struct Node {
    ...
    Node(const std::vector<float>& arr, int setId) : point(arr), id(setId), left(nullptr), right(nullptr) {}
...
private:
    ...
    void insertHelper(Node** node, std::size_t depth, const std::vector<float>& point, int idx) {
        if (*node == nullptr) {
    ...
    void searchHelper(const std::vector<float>& target, Node* node, int depth, float distanceTol,
                      std::vector<int>& ids) const {
        if (node != nullptr) {
            const float diffX = node->point[0] - target[0];
            const float diffY = node->point[1] - target[1];

            if (diffX >= -distanceTol && diffX <= distanceTol && diffY >= -distanceTol && diffY <= distanceTol) {
