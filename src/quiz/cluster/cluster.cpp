...
pcl::visualization::PCLVisualizer::Ptr initScene(const Box& window, int zoom) {
...
void render2DTree(Node* node, pcl::visualization::PCLVisualizer::Ptr& viewer, const Box& window, int& iteration,
                  std::size_t depth = 0) {
    if (node != nullptr) {
...
void clusterHelper(std::size_t index, const std::vector<std::vector<float>>& points, std::vector<int>& cluster,
                   std::vector<bool>& processed, KdTree* tree, float distanceTol) {
    processed.at(index) = true;

    cluster.push_back(index);

    const std::vector<int> nearest = tree->search(points.at(index), distanceTol);
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree,
    ...
    std::vector<bool> processed(nPoints, false);
