...
typename pcl::PointCloud<PointT>::Ptr ProcessPointClouds<PointT>::FilterCloud(
    typename pcl::PointCloud<PointT>::Ptr cloud, float filterRes, const Eigen::Vector4f& minPoint,
...
ProcessPointClouds<PointT>::SeparateClouds(pcl::PointIndices::Ptr inliers,
    ...
    for (const int idx : inliers->indices) {
        planeCloud->points.push_back(cloud->points.at(idx));
    }
    ...
    std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> segResult(obstaclesCloud,
                                                                                                      planeCloud);
...
ProcessPointClouds<PointT>::SegmentPlane(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations,
    ...
    pcl::SACSegmentation<PointT> seq;
    pcl::PointIndices::Ptr inliers(new pcl::PointIndices());
    pcl::ModelCoefficients::Ptr coefficients(new pcl::ModelCoefficients());

    seq.setOptimizeCoefficients(true);
    seq.setModelType(pcl::SACMODEL_PLANE);
    seq.setMethodType(pcl::SAC_RANSAC);
    seq.setMaxIterations(maxIterations);
    seq.setDistanceThreshold(distanceThreshold);

    seq.setInputCloud(cloud);

    seq.segment(*inliers, *coefficients);

    if (inliers->indices.size() == 0) {
        std::cout << "Could not estimate a planar nodel for the given dataset\n";
    }
